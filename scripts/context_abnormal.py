...
def prepare_random_batches(
        ...
        select_strands_randomly=True, first_strand_index=0, pin=True, right_len = 10, mod_num = None):
    total_sub_batches = 0
    ...
    while total_sub_batches < target_sub_batches:
        ...
        chunk_batch = sample_chunks_seq(
            read_data, sub_batch_size, batch_chunk_len, filter_params,alphabet_info,
            standardize=net_info.metadata.standardize,
            select_strands_randomly=select_strands_randomly,
            first_strand_index=first_strand_index, right_len = right_len, mod_num=mod_num)
        ...
        stacked_current = np.vstack([
            revop(chunk[0]) for chunk in chunk_batch]).T
        ...
        for chunk in chunk_batch:
            chunk_labels = revop(chunk[1])
            seqlens.append(len(chunk_labels))
            if net_info.metadata.is_cat_mod and alphabet_info.nbase > 4:
        ...
        yield indata, seqs, seqlens, mod_cats, sub_batch_size


def sample_chunks_seq(read_data, number_to_sample, chunk_len, filter_params,
                    alphabet_info,chunk_len_means_sequence_len=False,
                  standardize=True, select_strands_randomly=True,
                  first_strand_index=0, right_len = 0, mod_num = None):
    ...
    attempts = 0
    while(len(chunks) < number_to_sample_used):
        read_number = (
            np.random.randint(nreads) if select_strands_randomly else
            (first_strand_index + attempts) % nreads)
        attempts += 1
        read = read_data[read_number]
        read_canonical = np.array(list(alphabet_info.alphabet))[read.get_reference()]
        m_position = np.where(read_canonical==CAN_BASE)[0]
        if len(m_position) ==0: continue
        pos = int(np.random.choice(m_position,1))
        if pos < 30 or pos > len(read.get_reference())-30 or -1 in read.Ref_to_signal:
            continue
        winlen = int(chunk_len/2)
        if read.Ref_to_signal[pos]==-1:
        ...
        try:
            ...
            if len(signal)==chunk_len+right_len:
                if mod_num == None: chunks.append((signal,seq))
                elif np.count_nonzero(read.get_reference()[pos-5:pos+6]-alphabet_info.alphabet.index(CAN_BASE)) >= 11-mod_num:
                    chunks.append((signal,seq))
        except (RuntimeError, TypeError, NameError,IndexError):
            print("Overbound Error")
...
def calculate_loss(
    ...
    for (indata, seqs, seqlens, mod_cats, sub_batch_size) in batch_gen:
        n_subbatches += 1
...
def extract_reporting_data(
    ...
    reporting_chunk_len = args.sig_win_len*2
...
def train_model(
        ...
        filter_params, mod_info, winlen, reporting_batch_list, logs, batch_size=512,
        right_len=0, mod_num = None):
    ...
    for curr_iter in range(train_params.niteration):
        ...
        main_batch_gen = prepare_random_batches(
            ...
            logs.main,right_len = right_len, mod_num = mod_num)
...
def main(args):
    ...
    train_model(
        train_params, net_info, optim_info, res_info, read_data, alphabet_info,
        filter_params, mod_info, winlen, reporting_batch_list, logs, batch_size = args.min_sub_batch_size,
        right_len = args.right_len,mod_num = args.mod_num)
